...
// ── Warning list ──────────────────────────────────────────────────────────────
function loadAndRenderWarnings() {
  renderSavedWarnings(getWarnings());
}
...
function scheduleExpiryCheck() {
  ...
  const next = data.filter(item => {
    const exp = getExpiryTime(item);
    return exp === null || exp > now;
  });
  if (next.length !== data.length) {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
    closeAdminPopup();
    renderSavedWarnings(next);
  }

  const nexts = next.map(getExpiryTime).filter(t => t && t > now).sort((a, b) => a - b);
